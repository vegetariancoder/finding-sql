...
select
    ORDER_DAY,
    PRODUCT_ID,
    sum(QUANTITY*PRICE) as sum_values
from
    Order_Tbl
group by
...
select
    PRODUCT_ID,
    ifnull(sum(case
        when ORDER_DAY='2015-05-01' then sum_values
    end),0) as Total_Sales_01,
    ifnull(sum(case
        when ORDER_DAY='2015-05-02' then sum_values
    end),0) as Total_Sales_02
from
    answer
group by
    PRODUCT_ID;