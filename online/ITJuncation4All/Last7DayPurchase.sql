...
select * from Transactions_Amazon;
...
select
    UserId,
    CreatedAt,
    lead(CreatedAt) over (partition by UserId order by CreatedAt) as next_trans
from
    Transactions_Amazon
order by
    UserId,CreatedAt),
...
select
    *,
    if(datediff(next_trans,CreatedAt)<=7,'Yes','No') as checking
from
    answer)
select
    distinct UserId
